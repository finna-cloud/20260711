...
function settingsMarkup() {
    ...
    return `
        <section class="msa-page">
            ...
            <p class="msa-version">Midnight Signal APP · v1.2.0</p>
        </section>`;
}
...
function messagesMarkup() {
    ...
    return `
        <section class="msa-page msa-chat-page">
            <div class="msa-page-title"><span><small>MESSAGES</small><strong>與 ${escapeHtml(characterName(getCurrentCharacter()?.character))} 對話</strong></span></div>
...
async function sendMessageFromApp() {
    ...
    if (!nativeInput || !nativeSend) {
        notify('找不到 SillyTavern 原生聊天輸入框，請重新整理頁面後再試。', 'error');
        return;
    }
    if (nativeSend.disabled || nativeSend.classList.contains('displayNone')) {
