    ...
    return `
        <section class="msa-home">
            <button class="msa-hero" type="button" data-action="characters" aria-label="選擇對話角色">
                <img src="${import.meta.url.replace(/index\.js(?:\?.*)?$/, 'assets/signal-cover.png')}" alt="紅色聲波與雨夜城市">
            ...
            <button class="msa-continue" type="button" data-action="continue">${icon('comment-dots')}<span>繼續對話</span></button>
...
function settingsMarkup() {
    ...
    return `
        <section class="msa-page">
            ...
            <p class="msa-version">Midnight Signal APP · v1.0.0</p>
        </section>`;
}
        ...
        <section class="msa-page">
            ...
            <div class="msa-message-list">${chat.slice(-12).map(message => `
                <article class="msa-message ${message.is_user ? 'is-user' : 'is-character'}">
                    ...
                    <p>${escapeHtml(excerpt(message.mes, 180))}</p>
                </article>`).join('')}</div>
        </section>`;
}
